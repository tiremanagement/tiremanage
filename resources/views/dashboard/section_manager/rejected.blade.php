...
<div class="container-fluid py-4">
  ...
  <div class="row mb-4">
    <div class="col-md-8">
      ...
      <p class="text-muted">Review all rejected tire requests</p>
    </div>
    <div class="col-md-4">
        ...
        @forelse($requests as $req)
          ...
          <li class="request-card border-bottom">
            <div class="request-content p-4">
                  ...
                  <div class="d-flex align-items-center gap-3">
                    <div>
                      ...
                      <small class="text-muted">
                        <i class="bi bi-calendar3 me-1"></i>{{ optional($req->updated_at)->format('M d, Y H:i') ?? '-' }}
                      </small>
                    </div>
                  </div>
                </div>
              ...
              <!-- Rejection Reason -->
              @php
                $approval = $req->approvals()->where('status', \App\Models\Approval::STATUS_REJECTED)->first();
              @endphp
              @if($approval && $approval->remarks)
                <div class="mb-3">
                  <strong class="d-block mb-2">
                    <i class="bi bi-exclamation-circle me-1" style="color:#ef4444;"></i>Rejection Reason:
                  </strong>
                  <div style="background:#fef2f2; border-left:4px solid #ef4444; padding:12px; border-radius:0.35rem;">
                    <p style="margin:0; color:#333; white-space:pre-wrap;">{{ $approval->remarks }}</p>
                  </div>
                </div>
              @endif
...
document.addEventListener('DOMContentLoaded', () => {
  // Client-side filter for rejected list
